...
export class Collection<Type extends HasId> {
    ...
    updateRelations(relationsToUpdate: RelationUpdate[], relationsToRemove: RelationUpdate[]) {
    ...
    create(json: Partial<Type>) {
        ...
        Object.entries(this.belongsTo).forEach(([name, modelFn]) => {
            ...
            if (nameWithId in data) {
                relationsToUpdate.push({ name, newId, model: modelFn(), id: data[nameWithId] })
    ...
    pushRelation(model: string, id: Type['id'], relationId: Type['id'], force: string | null | undefined) {
    ...
    removeRelation(model: string, id: Type['id'], relationId: Type['id'], force?: string) {
        Object.entries(this.hasMany).forEach(([k, v]) => {
            if (force && k !== force) return;
            if (v() === model) {
                const item = this.find(id) as any;
    ...
    update(id: Type['id'], json: Partial<Type>) {
        ...
        Object.entries(this.belongsTo).forEach(([name, modelFn]) => {
            ...
            if (nameWithId in data) {
                if (data[nameWithId]) {
                    relationsToUpdate.push({ name, newId: idNormalised, model: modelFn(), id: data[nameWithId] })
                }
                if (oldId && oldId !== id) {
                    relationsToRemove.push({ name, newId: idNormalised, model: modelFn(), id: oldId })
                }
            } else if (name in data) {
                const foreignId = data[name].id;
                data[nameWithId] = data[name].id
                delete data[name];
                relationsToUpdate.push({ name, newId: idNormalised, model: modelFn(), id: foreignId })
                if (oldId && oldId !== id) {
                    relationsToRemove.push({ name, newId: idNormalised, model: modelFn(), id: oldId })
                }
            }
        ...
        Object.entries(this.hasMany).forEach(([name, modelFn]) => {
            ...
            if (nameWithId in data) {
                ...
                ids.filter((newId: ID) => !oldIds.includes(newId)).forEach((foreignId: ID) => {
                    relationsToUpdate.push({ name, newId: idNormalised, model: modelFn(), id: foreignId })
                })
                oldIds.filter((oldId: ID) => !ids.includes(oldId)).forEach((foreignId: ID) => {
                    relationsToRemove.push({ name, newId: idNormalised, model: modelFn(), id: foreignId })
                })
            } else if (name in data) {
                const ids = data[name].map((item: HasId) => item.id);
                data[nameWithId] = ids
...
export type VroomDb<Models, ModelId> = Database<FieldTypes<Models, { id: ModelId }>>;
