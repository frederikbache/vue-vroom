...
function createSingletonStore(name: string, baseURL = '', settings: StoreSettings) {
    const endpoint = settings.path ? baseURL + settings.path : `${baseURL}/${name}`;
...
function createStore(name: string, baseURL = '', settings: StoreSettings) {
    ...
    return defineStore('vroom:' + name, {
        ...
        getters: {
            list(state) {
                return (ids: ID[]) => state.items.filter(item => ids.includes(item.id))
            },
            single(state) {
        ...
        actions: {
            ...
            $list(filter: any, pagination: PaginationSettings, sort: SortSettings[], include: string[]) {
                let params = {
                    ...parseFilters(filter),
                    ...pagination,
                }
            ...
            update(id: ID, patchData: any) {
                return api.patch(endpoint + '/' + id, patchData).then(item => { this.add([item]) });
            },
            delete(id: ID) {
    ...
    return stores as {
        ...
        > : StoreDefinition<K,
            ...
            ItemActions<ModelInfo[K]['itemActions'], Type[K]['id']> &
            {
                ...
                // @ts-expect-error
                update: (id: Type[K]['id'], data: Partial<Type[K]>) => Promise<(Type[K])>,
                // @ts-expect-error
