...
function fetch() {
    state.value = 'loading'
    store.$single(props.id, props.include).then((res: any) => {
...
const item = computed(() => {
    ...
    props.include.forEach((rel: string) => {
        ...
        if (hasMany) {
            item[rel] = relStore.items.filter((relItem: any) => item[rel + 'Ids'].includes(relItem.id));
        } else {
            item[rel] = relStore.items.find((relItem: any) => item[rel + 'Id'] === relItem.id);
        }
    })
    return item;
})
