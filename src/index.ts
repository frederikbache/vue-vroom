...
export type FetchListComponent<Instance> = {
  new(): {
    ...
    $emit: ((event: 'ready', fetch: {
      refresh: () => void,
      create: (data: any) => void,
      // @ts-expect-error
      pushId: (id: Instance['settings'] extends { idsAreNumbers: true } ? number : string) => void
    }) => void),
    $slots: {
      default?: (props: {
        // @ts-expect-error
        ...
        meta: {
          // @ts-expect-error
          nextCursor?: Instance['settings'] extends { idsAreNumbers: true } ? number : string,
          page: number,
          pages: number,
          results: number,
          refresh: () => void,
