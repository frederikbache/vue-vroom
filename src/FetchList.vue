...
const relations = computed(() => ({ ...settings.hasMany, ...settings.belongsTo }))
const filterString = computed(() => JSON.stringify(props.filter) + '|' + JSON.stringify(props.pagination) + '|' + JSON.stringify(props.sort))

// States
const state = ref('none' as 'none' | 'loading' | 'updating' | 'failed');
const isLoading = computed(() => state.value === 'loading' && (!hasLoaded.value || props.loadOnUpdate));
...
const items = computed(() => {
    let all = store.list(ids.value).sort((a: any, b: any) => ids.value.indexOf(a.id) - ids.value.indexOf(b.id));
    props.include.forEach((rel: string) => {
        ...
        all = all.map((item: any) => {
            const newItem = { ...item }
            if (hasMany) {
                newItem[rel] = relStore.items.filter((relItem: any) => item[rel + 'Ids'].includes(relItem.id));
            } else {
                newItem[rel] = relStore.items.find((relItem: any) => item[rel + 'Id'] === relItem.id);
            }
            return newItem;
        })
    })
...
function fetch() {
    state.value = 'loading';
    store.$list(props.filter, props.pagination, props.sort, props.include).then((res: any) => {
